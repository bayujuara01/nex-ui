...
[NexProvider](/theming/mantine-provider) exposes all Mantine CSS variables based on the given [theme](/theming/theme-object).
...
```tsx
import { createTheme } from '@nex-ui/core';
...
```tsx
import { createTheme, DEFAULT_THEME } from '@nex-ui/core';
...
`cssVariablesResolver` prop on [NexProvider](/theming/mantine-provider) allows you to
