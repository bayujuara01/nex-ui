...
import type { EmotionStyles, EmotionSx } from '@nex-ui/emotion';

declare module '@nex-ui/core' {
...
Wrap your application with `MantineEmotionProvider` and add `emotionTransform` to `NexProvider`:
...
import { ColorSchemeScript, NexProvider } from '@nex-ui/core';
import {
  emotionTransform,
  MantineEmotionProvider,
} from '@nex-ui/emotion';
...
```tsx
import { Box, Button } from '@nex-ui/core';
...
```tsx
import { createTheme, MantineTheme, TextProps } from '@nex-ui/core';
import { EmotionHelpers } from '@nex-ui/emotion';
...
defined with [NexProvider](/theming/mantine-provider) or just static values:
