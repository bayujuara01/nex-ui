...
[NexProvider](/theming/mantine-provider) is required to be rendered at the root of your application.
...
function Demo() {
  return <NexProvider>{/* Your app here */}</NexProvider>;
}
```
...
```tsx
import { lighten, useNexTheme } from '@nex-ui/core';
...
You can configure the default color scheme on [NexProvider](/theming/mantine-provider):
