...
```tsx
import { AppShell, Burger } from '@nex-ui/core';
...
interface Configuration {
  ...
  breakpoint: NexBreakpoint | (string & {}) | number;
...
```tsx
import { AppShell, rem } from '@nex-ui/core';
...
function Demo() {
  ...
  return (
    ...
    >
      ...
      <AppShell.Main
        pt={`calc(${rem(60)} + var(--nex-spacing-md))`}
      >
        {/* Content */}
      </AppShell.Main>
    </AppShell>
...
```tsx
import { AppShell, Button } from '@nex-ui/core';
import { useDisclosure } from '@nex-ui/hooks';
...
```tsx
import { AppShell, ScrollArea } from '@nex-ui/core';
