...
both on [NexProvider](/theming/mantine-provider) and in [ColorSchemeScript](/theming/color-schemes#colorschemescript):
...
function Demo() {
  return (
    <>
      ...
      <NexProvider defaultColorScheme="auto">
        {/* Your app here */}
      </NexProvider>
    </>
  );
...
[NexProvider](/theming/mantine-provider) now comes with a built-in color scheme manager.
...
```tsx
import { generateColors } from '@nex-ui/colors-generator';
import { createTheme, NexProvider } from '@nex-ui/core';
