...
## New NexProvider props

[NexProvider](/theming/mantine-provider) now includes more props to control how styles
...
`HeadlessNexProvider` is an alternative to [NexProvider](/theming/mantine-provider)
...
Limitations of `HeadlessNexProvider`:
...
To use `HeadlessNexProvider`, follow the [getting started guide](/getting-started/) and replace `NexProvider` with `HeadlessNexProvider`.
...
```tsx
import { HeadlessNexProvider } from '@nex-ui/core';
...
- New [HeadlessNexProvider section](/styles/unstyled/#headlessNexProvider) in the unstyled guide
