...
To customize theme, pass theme override object to [NexProvider](/theming/mantine-provider/) `theme` prop.
Theme override will be deeply merged with the default theme.

```tsx
import { createTheme, NexProvider, rem } from '@nex-ui/core';
...
`useNexTheme` hook returns theme object from [NexProvider](/theming/mantine-provider) context:
...
[NexProvider](/theming/mantine-provider), it will be deeply merged with
