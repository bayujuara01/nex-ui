...
Add styles imports and [NexProvider](/theming/mantine-provider) to the `pages/_app.tsx` file:
...
import { ColorSchemeScript, mantineHtmlProps } from '@nex-ui/core';
...
Add [NexProvider](/theming/mantine-provider), [ColorSchemeScript](/theming/color-schemes)
...
import { ColorSchemeScript, NexProvider, mantineHtmlProps } from '@nex-ui/core';
...
export default function RootLayout({
  ...
  return (
    <html lang="en" {...mantineHtmlProps}>
      ...
      <body>
        <NexProvider>{children}</NexProvider>
      </body>
    </html>
  );
}
...
```tsx
import { Popover } from '@nex-ui/core';
...
export default {
  // ...other configuration
  experimental: {
    optimizePackageImports: ['@nex-ui/core', '@nex-ui/hooks'],
  },
};
```
