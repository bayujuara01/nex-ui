...
```tsx
import type { ButtonProps } from '@nex-ui/core';
import type { DatePickerProps } from '@nex-ui/dates';
```
...
```tsx
import { Group, GroupProps } from '@nex-ui/core';
...
```tsx
import { Button, ButtonProps, ElementProps } from '@nex-ui/core';
...
```tsx
import { ButtonProps, ElementProps } from '@nex-ui/core';
...
```tsx
import { MantineTheme, useNexTheme } from '@nex-ui/core';
...
`NexSize` type is a union of `'xs' | 'sm' | 'md' | 'lg' | 'xl'` values. You can use to add types
to various props that accept size as an argument, for example, `radius`, `shadow`, `p`.

```tsx
import { NexSize, Paper } from '@nex-ui/core';

interface DemoProps {
  size: NexSize;
  radius: NexSize | (string & {}) | number;
  shadow: NexSize | string;
}
...
```tsx
import { ButtonVariant, NexSize } from '@nex-ui/core';
