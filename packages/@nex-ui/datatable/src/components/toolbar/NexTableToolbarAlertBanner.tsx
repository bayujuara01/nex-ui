...
import classes from './NexTableToolbarAlertBanner.module.css';
...
import { NexTableSelectCheckbox } from '../inputs/NexTableSelectCheckbox';

interface Props<TData extends NexRowData> extends Partial<AlertProps> {
  stackAlertBanner?: boolean;
  table: NexTableTableInstance<TData>;
}

export const NexTableToolbarAlertBanner = <TData extends NexRowData>({
  stackAlertBanner,
  table,
  ...rest
}: Props<TData>) => {
        ...
        {renderToolbarAlertBannerContent?.({
          ...
          >
            {enableRowSelection &&
              enableSelectAll &&
              positionToolbarAlertBanner === 'head-overlay' && (
                <NexTableSelectCheckbox table={table} />
              )}
            <Stack>
              {alertProps?.children}
              {selectedAlert}
